...
void cEmpresa::AdquirirVehiculo(cVehiculo* vehiculo)
{
	if (vehiculo != NULL)
	{
		try {
			...
			string err = ex->what();
			delete ex;
			throw new runtime_error(("Error al agregar: " + err).c_str());
		}
	}
	else
		throw new runtime_error("Error al adquirir vehiculo: No hay datos del vehículo");
}
...
void cEmpresa::RetirardeCirculacion(cVehiculo* vehiculo)
{
	if (vehiculo != NULL)
	{
		...
		throw new runtime_error("Error al retirar de circulación: No hay datos del vehículo");
}
...
void cEmpresa::RetirardeCirculacion(string patente)
{
	...
	try {
		...
		string err = e->what();
		delete e;
		e = new runtime_error(("Error al buscar el vehiculo:" + err).c_str());
		throw e;
	}
