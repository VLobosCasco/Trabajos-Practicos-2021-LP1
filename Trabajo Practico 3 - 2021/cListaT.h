...
bool cListaT<T>::AgregarItem(T * item)
{
	...
	if (i_f != NULL)
		throw new runtime_error("Ya se encuentra en la lista");

	if (CA < TAM)
		vector[CA++] = item;
	else
		throw new runtime_error("No hay tamaño suficiente para agregar el item");
	return true;
}
...
T* cListaT<T>::QuitarenPos(unsigned int pos) {
	...
	if (pos >= CA || pos < 0)
		throw new runtime_error("Posicion invalida");
...
T* cListaT<T>::getItem(unsigned int pos)
{
	if (pos < CA && pos>=0)
		...
		throw new runtime_error("Posición inválida");
}
